    ...
    with open(tbf, 'r') as f:
        for l in f:
            ...
            if pident > lthd and pident < uthd:
                ...
                rgf_anir[contig_name].append(pident) # add pident to anir list
...
def calc_gene_stats(
    ...
    gTADs = [v for v in gene_tad.values()]
    avgTAD = sum(gTADs) / len(gTADs)

    print('... Writing Gene TAD file.')
    outfile = f'{outpre}_gene_tad.tsv'
    with open(outfile, 'w') as o:
        o.write(f'Name\tTAD{tad}\tNORM_TAD\tGene_Length\n')
        for k, v in gene_tad.items():
            n = v / avgTAD # Normalize TAD by avg gene TAD.
            o.write(f'{k}\t{v:.{precision}f}\t{n:.{precision}f}\t{gn_len[k]}\n')
...
def calc_genome_stats(
    mtg, wg_tad, wg_anir, wglen, tad, lthd, uthd, outpre, precision, bpmap
    ):
    ...
    if mtg is None:
        ...
        relabndc = (sum(wg_tad) / total_metagenome_bp) * 100
        total_relA = bpmap / total_metagenome_bp * 100
    else:
        ...
        total_relA = bpmap / total_metagenome_bp * 100
        total_relA = f'{total_relA:.{precision}f}%'
    ...
    wg_header = (
            ...
            f'TAD_Relative_Abundance(%)\tTotal_Relative_Abundance(%)\t'
            f'Genome_Length(bp)\tMetagenome_Length(bp)\n'
            )
    wg_lineout = (
            f'{outpre}\t{wgtad:.{precision}f}\t{wgbreadth:.{precision}f}\t'
            f'{wganir:.{precision}f}%\t'
            f'{relabndc}\t{total_relA}\t{wglen}\t{total_metagenome_bp}\n'
            )
...
def operator(
    ...
    rgf_tad, rgf_anir, bpmap = calc_genome_coverage(tbf, rgf_tad, lthd, uthd)
    ...
    if pgf:
        ...
        gn_tad, gn_len, intergn_tad, intergn_len
            ) = retrieve_prodigal_gene_coverage(pgf, rgf_tad)
        ...
        (
        gn_tad, gn_len, intergn_tad, intergn_len
            ) = retrieve_ncbi_gene_coverage(ncbi, rgf_tad)
    ...
    _ = calc_genome_stats(
        mtg, wg_tad, wg_anir, wglen, tadp, lthd, uthd, outpre, precision, bpmap
        )
