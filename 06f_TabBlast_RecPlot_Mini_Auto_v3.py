...
def func1(
    ...
    data = {
            'Sample': [], 'Location': [], 'Depth': [], 'pident': [],
            'position': [], 'rlen': [], 'alen': [], 'poslen': []
            }

    # Read through ref genome fasta and get genome length
    with open(fasta, 'r') as file:
        for name, seq in read_fasta(file):
            lengths[name[1:]] = genome_length # genome length by contig name
    ...
    popx_readmap_value_file.write('Sample\tpident above population threshold\n')
    # start plot
    sns.set_style("white")
    for samp in Sample_List:
        ...
        pop_cutoff = find_pop_threshold(
                                    subdf, samp, kde_bandwidth, valley_modifier
                                    )
        ...
        if len(popx) > 0:
            ...
            for poslen, posi in zip(popx['poslen'], popx['position']):
        ...
        if breadth >= breadth_cutoff:
            ...
            pident_string = ','.join([str(i) for i in popx.pident.tolist()])
            popx_readmap_values = f'{samp}\t{pident_string}\n'
