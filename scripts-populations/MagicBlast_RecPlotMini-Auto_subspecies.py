...
Sample, depth, and loca lines 160ish

## To merge all png files into a single pdf file in bash ##
convert directory/*pdf Filename.pdf
...
def func1(
    ...
    for samp in Sample_List:
        ...
        bns = [i for i in range(95,101)]
        _ = sns.histplot(
                    data=subdf, y="pident", bins=bns, ax=h.ax_marg_y, color=hcol
                    )


        h.ax_joint.axhline(y=anir, ls='--', color=lcol, lw=1)
        h.ax_joint.axhline(y=mnir, ls=':', color=lcol, lw=1)
...
def find_pop_threshold(subdf, samp, kde_bandwidth, valley_modifier, basefile):
    ...
    ax.set_xlim([95,100])
    ax.legend(loc='best')
    plt.savefig(f'{basefile}_Pop_Cutoff.pdf')
    plt.close()
...
def main():
    ...
    parser.add_argument(
        '-k', '--kde_bandwidth',
        help='Optional: Change bandwidth to control smoothing (default=0.1).',
        metavar='',
        type=float,
        required=False,
        default=0.1
    ...
    parser.add_argument(
        '-v', '--valley_modifier',
        help='Optional: Nudge the cutoff line (default=0.0).',
        metavar='',
        type=float,
        required=False,
        default=0.0
