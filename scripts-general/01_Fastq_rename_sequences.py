...
@{prefix}_Read_{readnum}_0000000000001
@{prefix}_Read_{readnum}_0000000000002
etc...
...
def main():
    ...
    parser.add_argument(
        '-p', '--read_name_prefix',
        help='Sample prefix to append to read names!',
        metavar='',
        type=str,
        required=True
        )
    parser.add_argument(
        '-r', '--paired_read_number',
        help='Please specify the paired read number ie 1 or 2!',
        metavar=':',
        type=str,
        required=False
        )
    ...
    readnum = args['paired_read_number']
    ...
    with open(infile, 'r') as f, open (outfile, 'w') as o:
        ...
        for name, seq, blank, qal in read_fastq(f):
            ...
            if readnum:
                line = (
                        f'@{prefix}_Read_{readnum}_{readcount:013}\n'
                        f'{seq}\n{blank}\n{qal}\n'
                        )
            else:
                line = f'@{prefix}_Read_{readcount:013}\n{seq}\n{blank}\n{qal}\n'
